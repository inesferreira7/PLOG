...
move_pawn(Xi,Yi,Xf,Yf,Bo):-
				...
				X = 3 -> (board(Bi) , convert(Yi, Numi), Indexi is (Numi - 1), replace(Bi,Indexi,Xi,vazio,Bint), convert(Yf,Numf), Indexf is (Numf - 1), replace(Bint,Indexf,Xf,pawn,Bo))
...
move_drone(Xi,Yi,Xf,Yf,Bo):-
				...
				(P1 = 3 , P2 = 3 )-> (write('move ok'), board(Bi), convert(Yi, Numi), Indexi is (Numi - 1), replace(Bi,Indexi,Xi,vazio,Bint), convert(Yf, Numf), Indexf is (Numf - 1), replace(Bint,Indexf,Xf,drone,Bo))
				).

test(Xi,Yi,Xf,Yf):-move_drone(Xi,Yi,Xf,Yf,Board), display_all(Board).
